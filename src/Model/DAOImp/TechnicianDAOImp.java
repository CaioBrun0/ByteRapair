...
import Model.DAOInter.TechnicianDAO;
...
public class TechnicianDAOImp implements TechnicianDAO {
    ...
    public void finishService(String CPF, String name) {
        invoiceID++;
        Model.DAOImp.OSDAOImp os2 = new Model.DAOImp.OSDAOImp();
        double value = 0;
        value = os2.finishAndValue(CPF);

        //GERANDO FATURA
        Date date = new Date();
        Model.DAOImp.InvoiceDAOImp invoice = new Model.DAOImp.InvoiceDAOImp();
        if (value > 0) {
            try {
